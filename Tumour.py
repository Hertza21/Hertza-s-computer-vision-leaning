...
name = 'C:/Users/Administrator/Desktop/Tumours/'+number+'.jpg'
#print(name)
img = cv2.imread(name)
#process pictures
...
#show(img1)
#identify cells
circles = cv2.HoughCircles(img1, cv2.HOUGH_GRADIENT_ALT, 1.5, 50, param1=1000, param2=0.2, minRadius=50, maxRadius=1000)
#mark
if str(circles) != 'None':
    for i in range(len(circles[0])):
        ...
        pt1 = (x-r,y-r)
        pt2 = (x+r,y+r)
        cv2.rectangle(img,pt1,pt2,(255,255,0),20,8) 
    #show(img)
    word = str('There are '+str(len(circles[0]))+' tumour cells in the picture.')
    if len(circles[0])<=2:
        word = str(word+'This sample is healthy')
    else:
        word = str(word+'This sample has a high possibility that have a tumour.')
else:
    word = 'There is no tumour cells in this picture.'+'This sample is healthy.'
font = cv2.FONT_HERSHEY_DUPLEX  # 设置字体
img = cv2.putText(img,word, (400, 200), font, 3, (255,255,255), 2)
#show(img)
cv2.imwrite('C:/Users/Administrator/Desktop/'+str(number)+'.jpg',img)
